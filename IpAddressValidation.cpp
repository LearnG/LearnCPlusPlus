...
std::vector<std::thread> g_threadObjects;
std::mutex m;
...
public:
	...
	void checkIPAddress(vector<string>& inputLines);
	void parsingInputFile();
...
//input - void
/////////////////////////////////////////////////////////////////////////////////////////////
void IpAddressValidation::checkIPAddress(std::vector<string>& inputLines) {
	g_threadObjects.push_back(std::thread([&](auto inputVector) {
		m.lock();
...
//input - void
/////////////////////////////////////////////////////////////////////////////////////////////
void IpAddressValidation::parsingInputFile()
{
	std::string line;

	ifstream input_file(inputFileName);
	if (!input_file.is_open()) {
		std::cerr << "Could not open the file - "<<inputFileName << "'" << endl;
		return;
	}
	...
	while (getline(input_file, line)) {
		...
		if (inputLines.size() == 5000) {
			checkIPAddress(inputLines);
			inputLines.clear();
		}
	}
